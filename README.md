...
回复可分成 1～4 个独立的 Telegram 消息气泡，以自然段为界，不强制拆句。亲密角色扮演只适用于明确成年的自愿参与者，具体内容仍受所选模型及中转站规则约束。
萌萌的日常语气由 `src/dialogue.js` 控制：承接上一句、回应具体事情，允许一段自然的话只用一个气泡。检测到明显客服套话时最多额外调用一次模型改写。旧版分隔标记仍能兼容，但不会要求模型继续输出标记。`/help` 查看指令。
萌萌默认可以更主动地调情、逗人和表达吸引；她会根据双方实际聊天和对方的回应调整分寸，而不是每轮套用同一种暧昧话术。
萌萌的角色起点是已经很喜欢当前聊天对象，不必等对方先示爱；这属于她的心意，不代表用户已经答应交往。她会在日常对话里表现私人偏爱，不只在主动调情时才表达喜欢。
启动时会同步 Telegram 底部命令菜单：`/start`、`/help`、`/partner`、`/memory`、`/reset`、`/forget`。
`/partner` 会只为当前 Telegram 用户保存“现实中有女朋友”的设定，`/partner off` 可关闭；直接告诉萌萌“我现实中有女朋友”也会记住。启用后萌萌在虚构聊天关系里明确想做第三者，会主动追求恋人般的亲近，同时尊重用户表达的停止和现实关系。`/forget` 同时删除这条个人设定。
